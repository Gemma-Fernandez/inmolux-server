...
router.get("/profile", verifyToken, async (req, res, next) => {
  try {
    ...
    const response = await User.findById(_id).select("-password"); //select no envia la contraseña
...
router.get("/vivienda/:viviendasId", verifyToken, async (req, res, next) => {
  try {
    ...
    const response = await User.find({ wishlist: viviendasId }).populate(
      "wishlist"
    );
    ...
    if (response.length === 0) {
      return res
        ...
        .json({
          message:
            "Esta vivienda aún no se ha añadido a ninguna whishlist :( ).",
        });
    }
    res.status(200).json(response);
  } catch (error) {
